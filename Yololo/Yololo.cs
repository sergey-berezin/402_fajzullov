        ...
        public static async Task<YoloRet> WorkImageAsync(Image<Rgb24> image, CancellationToken token)
        {
            ...
            while (!canc_token.IsCancellationRequested && session == null) {
                try
                {
                    ...
                    using var model_data = await client.GetStreamAsync("https://storage.yandexcloud.net/dotnet4/tinyyolov2-8.onnx");
                    using var file_model = new FileStream("tinyyolov2-8.onnx", FileMode.OpenOrCreate);
                    await model_data.CopyToAsync(file_model);
                }
            ...
            return new YoloRet(final, objects);
        }
    }

    public record YoloRet(Image<Rgb24> Image, IEnumerable<ObjectBox> BoxesInfo);
